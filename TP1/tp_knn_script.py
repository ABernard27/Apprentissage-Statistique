...
class KNNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        #  using e.g. sklearn.metrics.pairwise.pairwise_distances
        dist = np.sum((X[:, np.newaxis, :]-self.X_[np.newaxis, :, :])**2,
                      axis=-1)
        # sur la ligne de la matrice il y a les distances au point i
        ...
        return y_pred
# calcule les distances, les met dans le bon ordre puis recupere
# les k plus proches
...
knn75 = neighbors.KNeighborsClassifier(n_neighbors=75)
knn75.fit(X_train, Y_train)
plt.figure()
n_labels = np.unique(y2).shape[0]
# frontiere(knn1, X_train, Y_train, w=None, step=50, alpha_choice=1,
#               n_labels=n_labels, n_neighbors=1)
frontiere(knn1, X_train, Y_train, w=None, step=50, alpha_choice=1,
          n_labels=n_labels, n_neighbors=75)
# frontiere(knn1, X_train, Y_train, w=None, step=50, alpha_choice=1)
# frontiere(knn75, X_train, Y_train, w=None, step=50, alpha_choice=1)

# si k=n il prend la classe la plus nombreuse
# si k=1 il prend la classe du x le plus proche
...
# EC.errors les 10 erreurs de prédictions
EC.plot()
...
EC = ErrorCurve(k_range=list(range(1, 10, 1)))
EC.fit_curve(X_train, Y_train, X_test, Y_test)
# EC.errors les 10 erreurs de prédictions
EC.plot()
kmin2 = np.argmin(EC.errors)+1
print(kmin2)

# la meilleure valeure de k est celle pour laquelle on a la plus petite erreur
# %%
############################################################################
#     Digits data
############################################################################
...
loo_curve = LOOCurve(k_range=list(range(1, 50, 5)) +
                     list(range(100, 300, 100)))
