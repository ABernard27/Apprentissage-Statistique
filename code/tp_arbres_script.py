...
# Effectuer la classification d'un jeu de données simulées avec rand_checkers
# des échantillons de
...
# cette ligne appelle .fit qui mange X les données et Y les labels
# d'entrainements
dt_entropy.fit(X_train, Y_train)
# ça calcule les arbres de décision

print("Gini criterion")
print(dt_gini.get_params())
# donne les paramètres de la classe
print(dt_gini.score(X_train, Y_train))
# le .score calcule la précision qui est la performance du classifieur
# sur les données d'apprentissage, ici c'est la précision pas l'erreur
...
# %%
# Q3 Afficher la classification obtenue en utilisant la profondeur qui
# minimise le pourcentage d’erreurs
...
frontiere(lambda x: dt_entropy.predict(x.reshape((1, -1))), X_train, Y_train,
          step=100)
# ça crée une grille très fine, il affiche pour chaque point l'image du point
# par la fonction
# il rajoute les points de X avec les labels pour voir si c'est bien fait
# lambda : fonction qui a petit x associe predict de x
...
plt.xlabel('Max depth')
plt.ylabel('Accuracy Score')
plt.title('Score with entropy and gini criterion')
